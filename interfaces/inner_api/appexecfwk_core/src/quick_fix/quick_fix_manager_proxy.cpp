...
{
    LOG_I(BMSTag::QUICK_FIX, "create QuickFixManagerProxy.");
}

QuickFixManagerProxy::~QuickFixManagerProxy()
{
    LOG_I(BMSTag::QUICK_FIX, "destroy QuickFixManagerProxy.");
}
...
{
    LOG_I(BMSTag::QUICK_FIX, "begin to call DeployQuickFix.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);

    if (bundleFilePaths.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "DeployQuickFix failed due to params error.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }

    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        LOG_E(BMSTag::QUICK_FIX, "WriteInterfaceToken failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteStringVector(bundleFilePaths)) {
        LOG_E(BMSTag::QUICK_FIX, "write bundleFilePaths failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteBool(isDebug)) {
        LOG_E(BMSTag::QUICK_FIX, "write isDebug failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteRemoteObject(statusCallback->AsObject())) {
        LOG_E(BMSTag::QUICK_FIX, "write parcel failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }

    MessageParcel reply;
    if (!SendRequest(QuickFixManagerInterfaceCode::DEPLOY_QUICK_FIX, data, reply)) {
        LOG_E(BMSTag::QUICK_FIX, "SendRequest failed.");
...
{
    LOG_I(BMSTag::QUICK_FIX, "begin to call SwitchQuickFix.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);

    if (bundleName.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "SwitchQuickFix failed due to params error.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    ...
    if (!data.WriteString(bundleName)) {
        LOG_E(BMSTag::QUICK_FIX, "write bundleName failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteBool(enable)) {
        LOG_E(BMSTag::QUICK_FIX, "write enable failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteRemoteObject(statusCallback->AsObject())) {
        LOG_E(BMSTag::QUICK_FIX, "write parcel failed.");
...
ErrCode QuickFixManagerProxy::DeleteQuickFix(const std::string &bundleName,
    const sptr<IQuickFixStatusCallback> &statusCallback)
{
    LOG_I(BMSTag::QUICK_FIX, "begin to call DeleteQuickFix.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);

    if (bundleName.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "DeleteQuickFix failed due to params error.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
...
{
    LOG_D(BMSTag::QUICK_FIX, "begin to create fd.");
    if (fileName.empty()) {
        LOG_E(BMSTag::QUICK_FIX, "fileName is empty.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        LOG_E(BMSTag::QUICK_FIX, "write interface token failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteString(fileName)) {
        LOG_E(BMSTag::QUICK_FIX, "write fileName failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    MessageParcel reply;
    if (!SendRequest(QuickFixManagerInterfaceCode::CREATE_FD, data, reply)) {
        LOG_E(BMSTag::QUICK_FIX, "send request failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    auto ret = reply.ReadInt32();
    if (ret != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "reply return false.");
        return ret;
    }
    fd = reply.ReadFileDescriptor();
    if (fd < 0) {
        LOG_E(BMSTag::QUICK_FIX, "invalid fd.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_CREATE_FD_FAILED;
    }
    path = reply.ReadString();
    if (path.empty()) {
        LOG_E(BMSTag::QUICK_FIX, "invalid path.");
        close(fd);
        return ERR_BUNDLEMANAGER_QUICK_FIX_INVALID_TARGET_DIR;
    }
    LOG_D(BMSTag::QUICK_FIX, "create fd success.");
    return ERR_OK;
}
...
{
    LOG_D(BMSTag::QUICK_FIX, "begin to copy files.");
    if (sourceFiles.empty()) {
        LOG_E(BMSTag::QUICK_FIX, "sourceFiles empty.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    std::vector<std::string> hqfFilePaths;
    if (!BundleFileUtil::CheckFilePath(sourceFiles, hqfFilePaths)) {
        LOG_E(BMSTag::QUICK_FIX, "CopyFiles CheckFilePath failed");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    for (const std::string &sourcePath : hqfFilePaths) {
        size_t pos = sourcePath.find_last_of(SEPARATOR);
        if (pos == std::string::npos) {
            LOG_E(BMSTag::QUICK_FIX, "invalid sourcePath.");
            return ERR_BUNDLEMANAGER_QUICK_FIX_INVALID_PATH;
        }
        std::string fileName = sourcePath.substr(pos + 1);
        LOG_D(BMSTag::QUICK_FIX, "sourcePath:%{private}s fileName:%{private}s", sourcePath.c_str(), fileName.c_str());
        int32_t sourceFd = open(sourcePath.c_str(), O_RDONLY);
        if (sourceFd < 0) {
            LOG_E(BMSTag::QUICK_FIX, "open file failed, errno:%{public}d", errno);
        ...
        if ((ret != ERR_OK) || (destFd < 0) || (destPath.empty())) {
            LOG_E(BMSTag::QUICK_FIX, "create fd failed.");
            close(sourceFd);
            return ret;
        }
        ...
        while ((offset = read(sourceFd, buffer, sizeof(buffer))) > 0) {
            if (write(destFd, buffer, offset) < 0) {
                LOG_E(BMSTag::QUICK_FIX, "write file to the temp dir failed, errno %{public}d", errno);
                close(sourceFd);
                close(destFd);
    ...
    LOG_D(BMSTag::QUICK_FIX, "copy files success.");
    return ERR_OK;
}
...
bool QuickFixManagerProxy::SendRequest(QuickFixManagerInterfaceCode code, MessageParcel &data, MessageParcel &reply)
{
    ...
    if (remote == nullptr) {
        LOG_E(BMSTag::QUICK_FIX, "failed to send request %{public}d due to remote object null.", code);
        return false;
    }
    int32_t result = remote->SendRequest(static_cast<uint32_t>(code), data, reply, option);
    if (result != NO_ERROR) {
        LOG_E(BMSTag::QUICK_FIX, "receive error code %{public}d in transact %{public}d", result, code);
        return false;
    }
    return true;
}
