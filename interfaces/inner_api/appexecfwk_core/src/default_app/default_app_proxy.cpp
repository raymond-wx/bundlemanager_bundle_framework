...
{
    LOG_D(BMSTag::DEFAULT_APP, "create DefaultAppProxy.");
}

DefaultAppProxy::~DefaultAppProxy()
{
    LOG_D(BMSTag::DEFAULT_APP, "destroy DefaultAppProxy.");
}

ErrCode DefaultAppProxy::IsDefaultApplication(const std::string& type, bool& isDefaultApp)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to call IsDefaultApplication.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);

    MessageParcel data;
    if (!data.WriteInterfaceToken(GetDescriptor())) {
        LOG_E(BMSTag::DEFAULT_APP, "WriteInterfaceToken failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteString(type)) {
        LOG_E(BMSTag::DEFAULT_APP, "write type failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }

    MessageParcel reply;
    if (!SendRequest(DefaultAppInterfaceCode::IS_DEFAULT_APPLICATION, data, reply)) {
        LOG_E(BMSTag::DEFAULT_APP, "SendRequest failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to GetDefaultApplication.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);

    if (type.empty()) {
        LOG_E(BMSTag::DEFAULT_APP, "type is empty.");
    ...
    if (!data.WriteInt32(userId)) {
        LOG_E(BMSTag::DEFAULT_APP, "write userId failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (!data.WriteString(type)) {
        LOG_E(BMSTag::DEFAULT_APP, "write type failed.");
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to SetDefaultApplication.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);
    ...
    if (!data.WriteParcelable(&want)) {
        LOG_E(BMSTag::DEFAULT_APP, "write want failed.");
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to ResetDefaultApplication.");
    HITRACE_METER_NAME(HITRACE_TAG_APP, __PRETTY_FUNCTION__);
...
ErrCode DefaultAppProxy::GetParcelableInfo(DefaultAppInterfaceCode code, MessageParcel& data, T& parcelableInfo)
{
    ...
    ErrCode ret = reply.ReadInt32();
    if (ret != ERR_OK) {
        LOG_E(BMSTag::DEFAULT_APP, "host reply errCode : %{public}d", ret);
        return ret;
    }

    std::unique_ptr<T> info(reply.ReadParcelable<T>());
    if (info == nullptr) {
        LOG_E(BMSTag::DEFAULT_APP, "ReadParcelable failed.");
        return ERR_BUNDLE_MANAGER_INTERNAL_ERROR;
    }
    parcelableInfo = *info;
    LOG_D(BMSTag::DEFAULT_APP, "GetParcelableInfo success.");
    return ERR_OK;
}
...
bool DefaultAppProxy::SendRequest(DefaultAppInterfaceCode code, MessageParcel& data, MessageParcel& reply)
{
    ...
    if (remote == nullptr) {
        LOG_E(BMSTag::DEFAULT_APP, "failed to send request %{public}d due to remote object null.", code);
        return false;
    }
    int32_t result = remote->SendRequest(static_cast<uint32_t>(code), data, reply, option);
    if (result != NO_ERROR) {
        LOG_E(BMSTag::DEFAULT_APP, "receive error code %{public}d in transact %{public}d", result, code);
        return false;
    }
    return true;
}
