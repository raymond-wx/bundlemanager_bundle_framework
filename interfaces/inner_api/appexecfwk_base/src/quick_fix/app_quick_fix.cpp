...
void from_json(const nlohmann::json &jsonObject, AppQuickFix &appQuickFix)
{
    ...
    if (parseResult != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "read module appQuickFix from jsonObject error, error code : %{public}d", parseResult);
    }
}
...
bool AppQuickFix::ReadFromParcel(Parcel &parcel)
{
    ...
    if (!deployedAppqfInfoPtr) {
        LOG_E(BMSTag::QUICK_FIX, "ReadParcelable<AppqfInfo> failed");
        return false;
    }
