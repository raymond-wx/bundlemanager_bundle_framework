...
void from_json(const nlohmann::json &jsonObject, AppqfInfo &appqfInfo)
{
    ...
    if (parseResult != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "read module appqfInfo from jsonObject error, error code : %{public}d", parseResult);
    }
}
...
bool AppqfInfo::ReadFromParcel(Parcel &parcel)
{
    ...
    for (auto i = 0; i < hqfSize; i++) {
        ...
        if (!hqfInfoPtr) {
            LOG_E(BMSTag::QUICK_FIX, "ReadParcelable<HqfInfo> failed");
            return false;
        }
