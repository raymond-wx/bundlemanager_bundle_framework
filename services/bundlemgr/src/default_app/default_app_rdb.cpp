...
{
    LOG_D(BMSTag::DEFAULT_APP, "create DefaultAppRdb.");
...
DefaultAppRdb::~DefaultAppRdb()
{
    LOG_D(BMSTag::DEFAULT_APP, "destroy DefaultAppRdb.");
}

bool DefaultAppRdb::GetDefaultApplicationInfos(int32_t userId, std::map<std::string, Element>& infos)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to GetDefaultApplicationInfos, userId : %{public}d.", userId);
    bool ret = GetDataFromDb(userId, infos);
    if (!ret) {
        LOG_E(BMSTag::DEFAULT_APP, "GetDataFromDb failed.");
        return false;
    }

    LOG_D(BMSTag::DEFAULT_APP, "GetDefaultApplicationInfos success.");
    return true;
}

bool DefaultAppRdb::GetDefaultApplicationInfo(int32_t userId, const std::string& type, Element& element)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to GetDefaultApplicationInfo, userId : %{public}d, type : %{public}s.",
        userId, type.c_str());
    std::map<std::string, Element> infos;
    bool ret = GetDefaultApplicationInfos(userId, infos);
    if (!ret) {
        LOG_E(BMSTag::DEFAULT_APP, "GetDefaultApplicationInfos failed.");
        return false;
    }

    if (infos.find(type) == infos.end()) {
        LOG_D(BMSTag::DEFAULT_APP, "type is not saved in db.");
        return false;
    }

    element = infos.find(type)->second;
    LOG_D(BMSTag::DEFAULT_APP, "GetDefaultApplicationInfo success.");
    return true;
}

bool DefaultAppRdb::SetDefaultApplicationInfos(int32_t userId, const std::map<std::string, Element>& infos)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to SetDefaultApplicationInfos, userId : %{public}d.", userId);
    bool ret = SaveDataToDb(userId, infos);
    if (!ret) {
        LOG_E(BMSTag::DEFAULT_APP, "SaveDataToDb failed.");
        return false;
    }

    LOG_D(BMSTag::DEFAULT_APP, "SetDefaultApplicationInfos success.");
    return true;
}

bool DefaultAppRdb::SetDefaultApplicationInfo(int32_t userId, const std::string& type, const Element& element)
{
    LOG_D(BMSTag::DEFAULT_APP, "SetDefaultApplicationInfo userId:%{public}d type:%{public}s.", userId, type.c_str());
    std::map<std::string, Element> infos;
    GetDefaultApplicationInfos(userId, infos);
    if (infos.find(type) == infos.end()) {
        LOG_D(BMSTag::DEFAULT_APP, "add default app info.");
        infos.emplace(type, element);
    } else {
        LOG_D(BMSTag::DEFAULT_APP, "modify default app info.");
        infos[type] = element;
    }
    ...
    LOG_D(BMSTag::DEFAULT_APP, "SetDefaultApplicationInfo success.");
    return true;
}

bool DefaultAppRdb::DeleteDefaultApplicationInfos(int32_t userId)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to DeleteDefaultApplicationInfos, userId : %{public}d.", userId);
    bool ret = DeleteDataFromDb(userId);
    if (!ret) {
        LOG_E(BMSTag::DEFAULT_APP, "DeleteDataFromDb failed.");
        return false;
    }

    LOG_D(BMSTag::DEFAULT_APP, "DeleteDefaultApplicationInfos success.");
    return true;
}

bool DefaultAppRdb::DeleteDefaultApplicationInfo(int32_t userId, const std::string& type)
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to delete userId: %{public}d, type: %{public}s.", userId, type.c_str());
    std::map<std::string, Element> infos;
    bool ret = GetDataFromDb(userId, infos);
    if (!ret) {
        LOG_E(BMSTag::DEFAULT_APP, "GetDataFromDb failed.");
    ...
    if (infos.find(type) == infos.end()) {
        LOG_D(BMSTag::DEFAULT_APP, "type doesn't exists in db.");
        return true;
    }
    ...
    LOG_D(BMSTag::DEFAULT_APP, "DeleteDefaultApplicationInfo success.");
    return true;
}
...
bool DefaultAppRdb::ParseConfig(const std::string& relativePath, DefaultAppData& defaultAppData)
{
    ...
    if (rootDirs.empty()) {
        LOG_W(BMSTag::DEFAULT_APP, "rootDirs empty");
        return false;
    }
    std::for_each(rootDirs.cbegin(), rootDirs.cend(), [&relativePath, &defaultAppData](const auto& rootDir) {
        ...
        LOG_D(BMSTag::DEFAULT_APP, "default app json path : %{public}s", path.c_str());
        nlohmann::json jsonObject;
        if (!BundleParser::ReadFileIntoJson(path, jsonObject)) {
            LOG_W(BMSTag::DEFAULT_APP, "read default app json failed");
            return;
        }
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to LoadDefaultApplicationConfig.");
    DefaultAppData defaultAppData;
    if (!ParseConfig(DEFAULT_APP_JSON_PATH, defaultAppData)) {
        LOG_D(BMSTag::DEFAULT_APP, "default app config empty");
        return;
    }
    ...
    LOG_D(BMSTag::DEFAULT_APP, "LoadDefaultApplicationConfig done.");
}
...
bool DefaultAppRdb::GetDataFromDb(int32_t userId, std::map<std::string, Element>& infos)
{
    ...
    if (!result) {
        LOG_E(BMSTag::DEFAULT_APP, "QueryData failed by key %{public}d", userId);
        return false;
    }
    ...
    if (jsonObject.is_discarded() || defaultAppData.FromJson(jsonObject) != ERR_OK) {
        LOG_E(BMSTag::DEFAULT_APP, "error key : %{public}s", key.c_str());
...
{
    LOG_D(BMSTag::DEFAULT_APP, "RegisterDeathListener.");
}

void DefaultAppRdb::UnRegisterDeathListener()
{
    LOG_D(BMSTag::DEFAULT_APP, "UnRegisterDeathListener.");
}
}
}
