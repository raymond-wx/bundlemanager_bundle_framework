...
{
    LOG_D(BMSTag::DEFAULT_APP, "create DefaultAppMgr.");
    Init();
}

DefaultAppMgr::~DefaultAppMgr()
{
    LOG_D(BMSTag::DEFAULT_APP, "destroy DefaultAppMgr.");
...
ErrCode DefaultAppMgr::IsDefaultApplication(int32_t userId, const std::string& type, bool& isDefaultApp) const
{
    ...
    if (VerifyUserIdAndType(userId, mimeType) != ERR_OK) {
        LOG_W(BMSTag::DEFAULT_APP, "VerifyUserIdAndType failed.");
        isDefaultApp = false;
        return ERR_OK;
    }
    Element element;
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "GetDefaultApplicationInfo failed.");
        isDefaultApp = false;
        return ERR_OK;
    }
    ret = IsElementValid(userId, mimeType, element);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "Element is invalid.");
        isDefaultApp = false;
        return ERR_OK;
    }
    ...
    if (dataMgr == nullptr) {
        LOG_W(BMSTag::DEFAULT_APP, "get BundleDataMgr failed.");
        return ERR_BUNDLE_MANAGER_INTERNAL_ERROR;
    }
    std::string callingBundleName;
    ret = dataMgr->GetBundleNameForUid(IPCSkeleton::GetCallingUid(), callingBundleName);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "GetBundleNameForUid failed.");
        return ERR_BUNDLE_MANAGER_INTERNAL_ERROR;
    }
    LOG_D(BMSTag::DEFAULT_APP, "callingBundleName : %{public}s", callingBundleName.c_str());
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin, backup(bool) : %{public}d", backup);
    std::lock_guard<std::mutex> lock(mutex_);
    std::string mimeType = type;
    ConvertTypeBySuffix(mimeType);
    ...
    if (!BundlePermissionMgr::IsSystemApp()) {
        LOG_E(BMSTag::DEFAULT_APP, "non-system app calling system api");
    ...
    if (!BundlePermissionMgr::IsSelfCalling() &&
        ...
        LOG_W(BMSTag::DEFAULT_APP, "verify permission ohos.permission.GET_DEFAULT_APPLICATION failed.");
    ...
    if (IsAppType(mimeType)) {
        ...
        LOG_W(BMSTag::DEFAULT_APP, "invalid type, not app type or file type.");
...
ErrCode DefaultAppMgr::SetDefaultApplication(int32_t userId, const std::string& type, const Element& element) const
{
    ...
    if (!BundlePermissionMgr::IsSelfCalling() &&
        ...
        LOG_W(BMSTag::DEFAULT_APP, "verify permission ohos.permission.SET_DEFAULT_APPLICATION failed.");
        return ERR_BUNDLE_MANAGER_PERMISSION_DENIED;
    }
    ...
    if (ret) {
        LOG_D(BMSTag::DEFAULT_APP, "clear default app.");
        ret = defaultAppDb_->DeleteDefaultApplicationInfo(userId, mimeType);
        if (!ret) {
            LOG_W(BMSTag::DEFAULT_APP, "DeleteDefaultApplicationInfo failed.");
            return ERR_BUNDLE_MANAGER_ABILITY_AND_TYPE_MISMATCH;
        }
        LOG_D(BMSTag::DEFAULT_APP, "SetDefaultApplication success.");
        return ERR_OK;
    }
    ret = IsElementValid(userId, mimeType, element);
    if (!ret) {
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "SetDefaultApplicationInfo failed.");
        return ERR_BUNDLE_MANAGER_ABILITY_AND_TYPE_MISMATCH;
    }
    LOG_D(BMSTag::DEFAULT_APP, "SetDefaultApplication success.");
    return ERR_OK;
...
ErrCode DefaultAppMgr::ResetDefaultApplication(int32_t userId, const std::string& type) const
{
    ...
    if (!ret) {
        LOG_D(BMSTag::DEFAULT_APP, "directly delete default info.");
    ...
    LOG_D(BMSTag::DEFAULT_APP, "ResetDefaultApplication success.");
    return ERR_OK;
}
    ...
    for (const auto& item : currentInfos) {
        if (item.second.bundleName == bundleName) {
            ...
            if (defaultAppDb_->GetDefaultApplicationInfo(INITIAL_USER_ID, item.first, element)) {
                LOG_D(BMSTag::DEFAULT_APP, "set default application to preset, type : %{public}s", item.first.c_str());
                newInfos.emplace(item.first, element);
            } else {
                LOG_D(BMSTag::DEFAULT_APP, "erase uninstalled application type:%{public}s", item.first.c_str());
            }
        } else {
...
bool DefaultAppMgr::IsBrowserWant(const Want& want) const
{
    ...
    if (!matchAction) {
        LOG_D(BMSTag::DEFAULT_APP, "Action does not match, not browser want");
        return false;
    }
    std::string uri = want.GetUriString();
    bool matchUri = uri.rfind(HTTP_SCHEME, 0) == 0 || uri.rfind(HTTPS_SCHEME, 0) == 0;
    if (!matchUri) {
        LOG_D(BMSTag::DEFAULT_APP, "Uri does not match, not browser want");
        return false;
    }
    LOG_D(BMSTag::DEFAULT_APP, "is browser want");
    return true;
}
...
bool DefaultAppMgr::IsEmailWant(const Want& want) const
{
    ...
    if (!matchAction) {
        LOG_D(BMSTag::DEFAULT_APP, "Action does not match, not email want");
        return false;
    }
    std::string uri = want.GetUriString();
    bool matchUri = uri.rfind(EMAIL_SCHEME, 0) == 0;
    if (!matchUri) {
        LOG_D(BMSTag::DEFAULT_APP, "Uri does not match, not email want");
        return false;
    }
    LOG_D(BMSTag::DEFAULT_APP, "is email want");
    return true;
}
...
std::string DefaultAppMgr::GetType(const Want& want) const
{
    ...
    if (want.GetAction() == Constants::ACTION_VIEW_DATA) {
        ...
        if (MimeTypeMgr::GetMimeTypeByUri(uri, convertType)) {
            LOG_D(BMSTag::DEFAULT_APP, "get type by uri success, convertType : %{public}s", convertType.c_str());
            return convertType;
        }
        return Constants::EMPTY_STRING;
    }
    ...
    std::vector<AbilityInfo>& abilityInfos, std::vector<ExtensionAbilityInfo>& extensionInfos, bool backup) const
{
    ...
    LOG_I(BMSTag::DEFAULT_APP, "type : %{public}s", type.c_str());
    if (type.empty()) {
        LOG_E(BMSTag::DEFAULT_APP, "GetType failed");
        return false;
    }
    BundleInfo bundleInfo;
    ErrCode ret = GetDefaultApplication(userId, type, bundleInfo, backup);
    if (ret != ERR_OK) {
        LOG_E(BMSTag::DEFAULT_APP, "GetDefaultApplication failed");
        return false;
    }

    std::string bundleName = want.GetElement().GetBundleName();
    if (!bundleName.empty() && bundleName != bundleInfo.name) {
        LOG_D(BMSTag::DEFAULT_APP, "request bundleName : %{public}s, default bundleName : %{public}s",
    ...
    if (bundleInfo.abilityInfos.size() == 1) {
        ...
        LOG_D(BMSTag::DEFAULT_APP, "find default ability");
        return true;
    } else if (bundleInfo.extensionInfos.size() == 1) {
        extensionInfos = bundleInfo.extensionInfos;
        LOG_D(BMSTag::DEFAULT_APP, "find default extension");
        return true;
    } else {
        LOG_E(BMSTag::DEFAULT_APP, "invalid bundleInfo");
        return false;
    }
}
    ...
    int32_t userId, const std::string& type, BundleInfo& bundleInfo, bool backup) const
{
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "GetBundleInfo failed.");
        return ERR_BUNDLE_MANAGER_DEFAULT_APP_NOT_EXIST;
    }
    LOG_D(BMSTag::DEFAULT_APP, "GetBundleInfoByAppType success.");
    return ERR_OK;
}
    ...
    int32_t userId, const std::string& type, BundleInfo& bundleInfo, bool backup) const
{
    ...
    for (const auto& item : defaultAppTypeInfos) {
        ...
        for (const auto& mimeType : iter->second) {
            if (skill.MatchType(type, mimeType) && GetBundleInfo(userId, type, item.second, bundleInfo)) {
                LOG_D(BMSTag::DEFAULT_APP, "match default app type success");
                return ERR_OK;
            }
        }
    }
    ...
    for (const auto& item : defaultFileTypeInfos) {
        if (item.first == type && GetBundleInfo(userId, type, item.second, bundleInfo)) {
            LOG_D(BMSTag::DEFAULT_APP, "match default file type success.");
            return ERR_OK;
        }
    }
    LOG_W(BMSTag::DEFAULT_APP, "GetBundleInfoByFileType failed.");
    return ERR_BUNDLE_MANAGER_DEFAULT_APP_NOT_EXIST;
}
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to GetBundleInfo.");
    bool ret = VerifyElementFormat(element);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "VerifyElementFormat failed.");
        return false;
    }
    std::shared_ptr<BundleDataMgr> dataMgr = DelayedSingleton<BundleMgrService>::GetInstance()->GetDataMgr();
    if (dataMgr == nullptr) {
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "GetBundleInfo, QueryInfoAndSkillsByElement failed.");
        return false;
    }
    // match type and skills
    ret = IsMatch(type, skills);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "GetBundleInfo, type and skills not match.");
        return false;
    }
    ...
    LOG_D(BMSTag::DEFAULT_APP, "GetBundleInfo success.");
    return true;
}
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin, action : %{public}s, type : %{public}s", action.c_str(), type.c_str());
    ...
    LOG_W(BMSTag::DEFAULT_APP, "MatchActionAndType failed");
    return false;
}
...
bool DefaultAppMgr::IsMatch(const std::string& type, const std::vector<Skill>& skills) const
{
    if (IsAppType(type)) {
        ...
        LOG_W(BMSTag::DEFAULT_APP, "invalid type.");
        return false;
    }
}
...
{
    LOG_W(BMSTag::DEFAULT_APP, "begin to match app type, type : %{public}s.", type.c_str());
    if (type == BROWSER) {
    ...
    if (item == APP_TYPES.end()) {
        LOG_E(BMSTag::DEFAULT_APP, "invalid app type : %{public}s.", type.c_str());
        return false;
    }
...
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to verify browser skills.");
    Want httpWant;
    ...
    for (const Skill& skill : skills) {
        if (skill.Match(httpsWant) || skill.Match(httpWant)) {
            LOG_D(BMSTag::DEFAULT_APP, "browser skills is valid");
            return true;
        }
    }
    LOG_W(BMSTag::DEFAULT_APP, "browser skills is invalid.");
    return false;
}

bool DefaultAppMgr::IsEmailSkillsValid(const std::vector<Skill>& skills) const
{
    LOG_D(BMSTag::DEFAULT_APP, "begin to verify email skills");
    Want want;
    ...
    for (const Skill& skill : skills) {
        if (skill.Match(want)) {
            LOG_D(BMSTag::DEFAULT_APP, "email skills valid");
            return true;
        }
    }
    LOG_W(BMSTag::DEFAULT_APP, "email skills invalid");
    return false;
}

bool DefaultAppMgr::MatchFileType(const std::string& type, const std::vector<Skill>& skills) const
{
    LOG_D(BMSTag::DEFAULT_APP, "type : %{public}s", type.c_str());
    if (MatchActionAndType(Constants::ACTION_VIEW_DATA, type, skills)) {
        return true;
    }
    LOG_E(BMSTag::DEFAULT_APP, "MatchFileType failed");
    return false;
}
...
bool DefaultAppMgr::IsFileType(const std::string& type) const
{
    ...
    if (type.empty() || type.find(WILDCARD) != type.npos) {
        LOG_W(BMSTag::DEFAULT_APP, "file type not allow contains *.");
        return false;
    }
    ...
    LOG_W(BMSTag::DEFAULT_APP, "file type format invalid.");
    return false;
}
...
ErrCode DefaultAppMgr::VerifyUserIdAndType(int32_t userId, const std::string& type) const
{
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "userId %{public}d doesn't exist.", userId);
        return ERR_BUNDLE_MANAGER_INVALID_USER_ID;
    }
    ret = IsTypeValid(type);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "invalid type %{public}s, not app type or file type.", type.c_str());
        return ERR_BUNDLE_MANAGER_INVALID_TYPE;
    }
    return ERR_OK;
}
...
bool DefaultAppMgr::VerifyElementFormat(const Element& element)
{
    ...
    if (bundleName.empty()) {
        LOG_W(BMSTag::DEFAULT_APP, "bundleName empty, bad Element format.");
        return false;
    }
    if (moduleName.empty()) {
        LOG_W(BMSTag::DEFAULT_APP, "moduleName empty, bad Element format.");
        return false;
    }
    if (abilityName.empty() && extensionName.empty()) {
        LOG_W(BMSTag::DEFAULT_APP, "abilityName and extensionName both empty, bad Element format.");
        return false;
    }
    if (!abilityName.empty() && !extensionName.empty()) {
        LOG_W(BMSTag::DEFAULT_APP, "abilityName and extensionName both non-empty, bad Element format.");
        return false;
    }
    return true;
}
...
bool DefaultAppMgr::IsElementValid(int32_t userId, const std::string& type, const Element& element) const
{
    ...
    ret = dataMgr->QueryInfoAndSkillsByElement(userId, element, abilityInfo, extensionInfo, skills);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "QueryInfoAndSkillsByElement failed.");
        return false;
    }
    // match type and skills
    ret = IsMatch(type, skills);
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "type and skills not match.");
        return false;
    }
    LOG_D(BMSTag::DEFAULT_APP, "Element is valid.");
    return true;
}
...
void DefaultAppMgr::ConvertTypeBySuffix(std::string& suffix) const
{
    if (suffix.empty() || suffix.find('.') != 0) {
        LOG_D(BMSTag::DEFAULT_APP, "default app type is not suffix form");
        return;
    }
    ...
    if (!ret) {
        LOG_W(BMSTag::DEFAULT_APP, "uri suffix %{public}s has no corresponding mime type", suffix.c_str());
        return;
    }
    LOG_D(BMSTag::DEFAULT_APP, "corresponding mime type is %{public}s", type.c_str());
    suffix = type;
    return;
}
}
