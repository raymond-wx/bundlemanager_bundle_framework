...
{
    LOG_I(BMSTag::QUICK_FIX, "start to process quick fix boot up");
    ...
    if (!ret || quickFixInfoMap.empty()) {
        LOG_W(BMSTag::QUICK_FIX, "no quick fix info in db");
        RestoreQuickFix();
        return;
    }
    ...
    for (const auto &quickFixInfo : quickFixInfoMap) {
        ...
        LOG_D(BMSTag::QUICK_FIX, "quick fix scan bundle %{public}s, status is %{public}d",
        ...
        if (res != ERR_OK) {
            LOG_E(BMSTag::QUICK_FIX, "quick fix info %{public}s is processed failed with error %{public}d",
    ...
    RemoveInvalidDir();
    LOG_I(BMSTag::QUICK_FIX, "process quick fix boot up completely");
}
...
{
    if (state_ == nullptr) {
        LOG_E(BMSTag::QUICK_FIX, "state_ is nullptr");
...
{
    LOG_I(BMSTag::QUICK_FIX, "start to RestoreQuickFix");
    std::vector<std::string> dirVec;
    if (InstalldClient::GetInstance()->ObtainQuickFixFileDir(Constants::BUNDLE_CODE_DIR, dirVec) != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "RestoreQuickFix failed due to obtained quick fix file dir failed");
        return;
    }
    ProcessQuickFixDir(dirVec);

    if (quickFixInfoMap_.empty()) {
        LOG_W(BMSTag::QUICK_FIX, "no quick fix info in quickFixInfoMap_");
        return;
    }
    for (const auto &quickFix : quickFixInfoMap_) {
        ...
        if (!GetApplicationInfo(quickFix.first, quickFix.second.second, appInfo)) {
            LOG_W(BMSTag::QUICK_FIX, "appInfo is no existed, the quick info file need to be deleted");
            continue;
        }
        ...
        if (qfInfo.hqfInfos.empty()) {
            LOG_D(BMSTag::QUICK_FIX, "no quikc fix info in the appInfo and reprocess the quick fix");
            if (!ReprocessQuickFix(quickFix.second.second, quickFix.first)) {
                LOG_E(BMSTag::QUICK_FIX, "ReprocessQuickFix failed");
            }
            continue;
        }
        // 3. appInfo contain quick fix info, there need to check the quick fix info version with the
        //    quick fix file version code
        LOG_D(BMSTag::QUICK_FIX, "appInfo contains quick fix info");
        ...
        if (!ProcessWithBundleHasQuickFixInfo(quickFix.first, quickFix.second.second, quickFixVersion,
            ...
            LOG_E(BMSTag::QUICK_FIX, "ProcessWithBundleHasQuickFixInfo failed");
        }
    }

    RemoveInvalidDir();
    LOG_I(BMSTag::QUICK_FIX, "calling RestoreQuickFix successfully");
}

void QuickFixBootScanner::ProcessQuickFixDir(const std::vector<std::string> &fileDir)
{
    LOG_I(BMSTag::QUICK_FIX, "start to ObtainQuickFixInfo");
    if (fileDir.empty()) {
        LOG_W(BMSTag::QUICK_FIX, "no quick fix file");
        return;
    }
    for (const auto &fileStr : fileDir) {
        LOG_D(BMSTag::QUICK_FIX, "ObtainQuickFixInfo fileStr is %{public}s", fileStr.c_str());
        size_t underlinePos = fileStr.rfind(Constants::FILE_UNDERLINE);
        if (underlinePos == std::string::npos) {
            LOG_E(BMSTag::QUICK_FIX, "ObtainQuickFixInfo failed due to invalid dir");
        ...
        if (!ret) {
            LOG_E(BMSTag::QUICK_FIX, "ObtainQuickFixInfo failed due to invalid versionCode in dir");
            invalidQuickFixDir_.emplace_back(fileStr);
            continue;
        }
        LOG_D(BMSTag::QUICK_FIX, "versionCode of the quick fix file is %{public}d", versionCode);
        ...
        LOG_D(BMSTag::QUICK_FIX, "bundleName of the quick fix file is %{public}s", bundleName.c_str());
...
{
    LOG_D(BMSTag::QUICK_FIX, "start to ReprocessQuickFix with bundleName %{public}s", bundleName.c_str());
    ...
    if (!BundleUtil::CreateDir(destinationDir)) {
        LOG_E(BMSTag::QUICK_FIX, "create dir failed");
        return false;
    }
    if (InstalldClient::GetInstance()->CopyFiles(quickFixPath, destinationDir) != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "RestoreQuickFix failed due to copy quick fix files failed");
        return false;
    }
    ...
    ApplicationInfo &info)
{
    ...
    if (!dataMgr_->GetApplicationInfo(bundleName, ApplicationFlag::GET_BASIC_APPLICATION_INFO, Constants::ANY_USERID,
        info)) {
        LOG_W(BMSTag::QUICK_FIX, "application info is no existed, the quick info file need to be deleted");
    ...
    int32_t quickFixVersion, int32_t fileVersion)
{
    if (quickFixVersion == fileVersion) {
        LOG_D(BMSTag::QUICK_FIX, "same version code between quick fix file and quick fix info");
        ...
        LOG_I(BMSTag::QUICK_FIX, "invalid the quick fix file dir and quick fix info needs to be remove");
    }
    return true;
}
