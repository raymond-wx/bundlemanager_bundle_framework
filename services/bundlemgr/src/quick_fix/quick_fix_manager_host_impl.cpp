...
{
    LOG_I(BMSTag::QUICK_FIX, "create QuickFixManagerHostImpl");
}

QuickFixManagerHostImpl::~QuickFixManagerHostImpl()
{
    LOG_I(BMSTag::QUICK_FIX, "destory QuickFixManagerHostImpl");
}
...
{
    LOG_I(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeployQuickFix start");
    if (bundleFilePaths.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeployQuickFix wrong parms");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    if (!BundlePermissionMgr::IsSystemApp()) {
        LOG_E(BMSTag::QUICK_FIX, "non-system app is not allowed call this function");
    ...
    if (!BundlePermissionMgr::VerifyCallingPermissionForAll(Constants::PERMISSION_INSTALL_BUNDLE) &&
        ...
        LOG_E(BMSTag::QUICK_FIX, "verify install permission failed.");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PERMISSION_DENIED;
    }
    if (!GetQuickFixMgr()) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeployQuickFix quickFixerMgr is nullptr");
        return ERR_BUNDLEMANAGER_QUICK_FIX_INTERNAL_ERROR;
    }
    std::vector<std::string> securityFilePaths;
    ErrCode result = CopyHqfToSecurityDir(bundleFilePaths, securityFilePaths);
    if (result != ERR_OK) {
        LOG_E(BMSTag::QUICK_FIX, "copy file to secure dir failed %{public}d", result);
        return result;
    }
...
{
    LOG_I(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::SwitchQuickFix start");
    if (bundleName.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::SwitchQuickFix wrong parms");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    if (!BundlePermissionMgr::IsSystemApp()) {
        LOG_E(BMSTag::QUICK_FIX, "non-system app is not allowed call this function");
    ...
    if (!GetQuickFixMgr()) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::SwitchQuickFix quickFixerMgr is nullptr");
        return ERR_BUNDLEMANAGER_QUICK_FIX_INTERNAL_ERROR;
    }
...
ErrCode QuickFixManagerHostImpl::DeleteQuickFix(const std::string &bundleName,
    const sptr<IQuickFixStatusCallback> &statusCallback)
{
    LOG_I(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeleteQuickFix start");
    if (bundleName.empty() || (statusCallback == nullptr)) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeleteQuickFix wrong parms");
        return ERR_BUNDLEMANAGER_QUICK_FIX_PARAM_ERROR;
    }
    if (!BundlePermissionMgr::IsSystemApp()) {
        LOG_E(BMSTag::QUICK_FIX, "non-system app is not allowed call this function");
    ...
    if (!GetQuickFixMgr()) {
        LOG_E(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::DeleteQuickFix quickFixerMgr is nullptr");
        return ERR_BUNDLEMANAGER_QUICK_FIX_INTERNAL_ERROR;
    }
...
{
    LOG_D(BMSTag::QUICK_FIX, "QuickFixManagerHostImpl::CreateFd start.");
    if (!BundlePermissionMgr::IsSystemApp()) {
        LOG_E(BMSTag::QUICK_FIX, "non-system app is not allowed call this function");
        return ERR_BUNDLE_MANAGER_SYSTEM_API_DENIED;
    }
...
{
    LOG_D(BMSTag::QUICK_FIX, "start to copy hqf files to securityFilePaths");
    ...
    for (const auto &path : bundleFilePaths) {
        if (path.find(prefixStr) == std::string::npos) {
            LOG_E(BMSTag::QUICK_FIX, "invalid hqf path %{public}s", path.c_str());
        ...
        LOG_D(BMSTag::QUICK_FIX, "copy hqf file from path(%{public}s) to securePath(%{public}s)",
            path.c_str(), securityPath.c_str());
        if (!BundleUtil::CopyFile(path, securityPath)) {
            LOG_E(BMSTag::QUICK_FIX, "CopyFile failed");
