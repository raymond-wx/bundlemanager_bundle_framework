...
{
    LOG_I(BMSTag::QUICK_FIX, "create QuickFixManagerRdb.");
...
QuickFixManagerRdb::~QuickFixManagerRdb()
{
    LOG_I(BMSTag::QUICK_FIX, "destroy QuickFixManagerRdb.");
}

bool QuickFixManagerRdb::QueryAllInnerAppQuickFix(std::map<std::string, InnerAppQuickFix> &innerAppQuickFixes)
{
    LOG_I(BMSTag::QUICK_FIX, "begin to QueryAllInnerAppQuickFix");
    bool ret = GetAllDataFromDb(innerAppQuickFixes);
    if (!ret) {
        LOG_E(BMSTag::QUICK_FIX, "GetAllDataFromDb failed.");
        return false;
    }
    return true;
}
...
{
    LOG_I(BMSTag::QUICK_FIX, "begin to QueryAppQuickFix");
    bool ret = GetDataFromDb(bundleName, innerAppQuickFix);
    if (!ret) {
        LOG_E(BMSTag::QUICK_FIX, "GetDataFromDb failed.");
        return false;
    }
    return true;
}
...
{
    LOG_I(BMSTag::QUICK_FIX, "begin to SaveInnerAppQuickFix");
    bool ret = SaveDataToDb(innerAppQuickFix);
    if (!ret) {
        LOG_E(BMSTag::QUICK_FIX, "SaveDataToDb failed.");
        return false;
    }
    return true;
}
...
{
    LOG_I(BMSTag::QUICK_FIX, "begin to DeleteInnerAppQuickFix");
    bool ret = DeleteDataFromDb(bundleName);
    if (!ret) {
        LOG_E(BMSTag::QUICK_FIX, "DeleteDataFromDb failed.");
        return false;
    }
    return true;
}
...
bool QuickFixManagerRdb::GetAllDataFromDb(std::map<std::string, InnerAppQuickFix> &innerAppQuickFixes)
{
    if (rdbDataManager_ == nullptr) {
        LOG_E(BMSTag::QUICK_FIX, "rdbDataManager is null");
        return false;
    }
    ...
    if (!result) {
        LOG_E(BMSTag::QUICK_FIX, "QueryAllData failed");
        return false;
    }
    for (auto iter = values.begin(); iter != values.end(); ++iter) {
        ...
        if (jsonObject.is_discarded() || (appQuickFix.FromJson(jsonObject) != ERR_OK)) {
            LOG_E(BMSTag::QUICK_FIX, "error key : %{public}s", iter->first.c_str());
...
bool QuickFixManagerRdb::GetDataFromDb(const std::string &bundleName, InnerAppQuickFix &innerAppQuickFix)
{
    ...
    if (!result) {
        LOG_E(BMSTag::QUICK_FIX, "QueryData failed by bundleName %{public}s", bundleName.c_str());
        return false;
    }
    ...
    if (jsonObject.is_discarded() || (innerAppQuickFix.FromJson(jsonObject) != ERR_OK)) {
        LOG_E(BMSTag::QUICK_FIX, "error key : %{public}s", bundleName.c_str());
