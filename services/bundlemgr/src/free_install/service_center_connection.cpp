...
{
    LOG_I(BMSTag::FREE_INSTALL, "ServiceCenterConnection destory");
}

void ServiceCenterConnection::OnAbilityConnectDone(
    const AppExecFwk::ElementName &element, const sptr<IRemoteObject> &remoteObject, int32_t resultCode)
{
    LOG_I(BMSTag::FREE_INSTALL, "OnAbilityConnectDone start");
    if (resultCode != ERR_OK) {
        LOG_E(BMSTag::FREE_INSTALL, "OnAbilityConnectDone resultCode = %{public}d", resultCode);
    ...
    if (remoteObject == nullptr) {
        LOG_E(BMSTag::FREE_INSTALL, "OnAbilityConnectDone remoteObject is nullptr");
        connectState_ = ServiceCenterConnectState::DISCONNECTED;
        cv_.notify_all();
        return;
    }
    ...
    if (deathRecipient_ == nullptr) {
        LOG_E(BMSTag::FREE_INSTALL, "Failed to create ServiceCenterDeathRecipient");
        cv_.notify_all();
        return;
    }

    if (!serviceCenterRemoteObject_->AddDeathRecipient(deathRecipient_)) {
        LOG_E(BMSTag::FREE_INSTALL, "Failed to add AbilityManagerDeathRecipient");
    }
    cv_.notify_all();
    LOG_I(BMSTag::FREE_INSTALL, "OnAbilityConnectDone end");
}

void ServiceCenterConnection::OnAbilityDisconnectDone(const AppExecFwk::ElementName &element, int32_t resultCode)
{
    LOG_I(BMSTag::FREE_INSTALL, "OnAbilityDisconnectDone start");
    ...
    cv_.notify_all();
    LOG_I(BMSTag::FREE_INSTALL, "OnAbilityDisconnectDone end");
}
