...
{
    LOG_D(BMSTag::APP_CONTROL, "create AppControlManagerRdb.");
...
AppControlManagerRdb::~AppControlManagerRdb()
{
    LOG_D(BMSTag::APP_CONTROL, "destroy AppControlManagerRdb.");
}
    ...
    const std::vector<std::string> &appIds, const std::string &controlRuleType, int32_t userId)
{
    ...
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "BatchInsert failed.");
    ...
    const std::string &controlRuleType, const std::vector<std::string> &appIds, int32_t userId)
{
    for (const auto &appId : appIds) {
        ...
        if (!ret) {
            LOG_E(BMSTag::APP_CONTROL, "Delete failed callingName:%{public}s appId:%{public}s userId:%{public}d",
    ...
    const std::string &controlRuleType, int32_t userId)
{
    ...
    bool ret = rdbDataManager_->DeleteData(absRdbPredicates);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteData callingName:%{public}s controlRuleType:%{public}s failed.",
    ...
    const std::string &controlRuleType, int32_t userId, std::vector<std::string> &appIds)
{
    ...
    if (absSharedResultSet == nullptr) {
        LOG_E(BMSTag::APP_CONTROL, "GetAppInstallControlRule failed.");
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    ...
    if (count == 0) {
        LOG_D(BMSTag::APP_CONTROL, "GetAppInstallControlRule size 0");
        return ERR_OK;
    }
    ...
    do {
        ...
        if (ret != NativeRdb::E_OK) {
            LOG_E(BMSTag::APP_CONTROL, "GetString appId failed, ret: %{public}d", ret);
            return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
        }
    ...
    const std::vector<AppRunningControlRule> &controlRules, int32_t userId)
{
    ...
    bool ret = rdbDataManager_->BatchInsert(insertNum, valuesBuckets);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "BatchInsert AddAppRunningControlRule failed.");
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    if (valuesBuckets.size() != static_cast<uint64_t>(insertNum)) {
        LOG_E(BMSTag::APP_CONTROL, "BatchInsert size not expected.");
...
ErrCode AppControlManagerRdb::DeleteAppRunningControlRule(const std::string &callingName, int32_t userId)
{
    ...
    bool ret = rdbDataManager_->DeleteData(absRdbPredicates);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteAppRunningControlRule callingName:%{public}s userId:%{public}d failed.",
    ...
    int32_t userId, std::vector<std::string> &appIds)
{
    ...
    if (count == 0) {
        LOG_D(BMSTag::APP_CONTROL, "GetAppRunningControlRule size 0");
        return ERR_OK;
    }
    ret = absSharedResultSet->GoToFirstRow();
    if (ret != NativeRdb::E_OK) {
    ...
    int32_t userId, AppRunningControlRuleResult &controlRuleResult)
{
    ...
    if (count == 0) {
        LOG_W(BMSTag::APP_CONTROL, "invalid size 0");
    ...
    if (absSharedResultSet->GetString(CALLING_NAME_INDEX, callingName) != NativeRdb::E_OK) {
        LOG_E(BMSTag::APP_CONTROL, "GetString callingName failed, ret: %{public}d", ret);
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    ret = absSharedResultSet->GetString(CONTROL_MESSAGE_INDEX, controlRuleResult.controlMessage);
    if (ret != NativeRdb::E_OK) {
        LOG_W(BMSTag::APP_CONTROL, "GetString controlMessage failed, ret: %{public}d", ret);
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    if (controlRuleResult.controlMessage.empty() && callingName == AppControlConstants::EDM_CALLING) {
        LOG_D(BMSTag::APP_CONTROL, "GetString controlMessage default");
    ...
    if (absSharedResultSet->GetString(DISPOSED_STATUS_INDEX, wantString) != NativeRdb::E_OK) {
        LOG_E(BMSTag::APP_CONTROL, "GetString controlWant failed, ret: %{public}d", ret);
    }
    if (!wantString.empty()) {
...
{
    LOG_D(BMSTag::APP_CONTROL, "rdb begin to SetDisposedStatus");
    ErrCode code = DeleteDisposedStatus(callingName, appId, userId);
    if (code != ERR_OK) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteDisposedStatus failed.");
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    int64_t timeStamp = BundleUtil::GetCurrentTime();
    NativeRdb::ValuesBucket valuesBucket;
    ...
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "SetDisposedStatus callingName:%{public}s appId:%{public}s failed.",
...
{
    LOG_D(BMSTag::APP_CONTROL, "rdb begin to DeleteDisposedStatus");
    NativeRdb::AbsRdbPredicates absRdbPredicates(APP_CONTROL_RDB_TABLE_NAME);
    absRdbPredicates.EqualTo(CALLING_NAME, callingName);
    absRdbPredicates.EqualTo(APP_CONTROL_LIST, RUNNING_CONTROL);
    absRdbPredicates.EqualTo(APP_ID, appId);
    ...
    bool ret = rdbDataManager_->DeleteData(absRdbPredicates);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteDisposedStatus callingName:%{public}s appId:%{public}s failed.",
            callingName.c_str(), appId.c_str());
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    return ERR_OK;
...
{
    LOG_D(BMSTag::APP_CONTROL, "rdb begin to GetDisposedStatus");
    NativeRdb::AbsRdbPredicates absRdbPredicates(APP_CONTROL_RDB_TABLE_NAME);
    absRdbPredicates.EqualTo(CALLING_NAME, callingName);
    absRdbPredicates.EqualTo(APP_CONTROL_LIST, RUNNING_CONTROL);
    absRdbPredicates.EqualTo(APP_ID, appId);
    ...
    if (ret != NativeRdb::E_OK) {
        LOG_E(BMSTag::APP_CONTROL, "GetString DisposedStatus failed, ret: %{public}d", ret);
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    ...
    const std::string &appId, int32_t userId)
{
    ...
    bool ret = rdbDataManager_->DeleteData(absRdbPredicates);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteOldControlRule %{public}s, %{public}s, %{public}s, %{public}d failed.",
...
ErrCode AppControlManagerRdb::DeleteAllDisposedRuleByBundle(const std::string &appId, int32_t userId)
{
    ...
    bool ret = rdbDataManager_->DeleteData(absRdbPredicates);
    if (!ret) {
        LOG_E(BMSTag::APP_CONTROL, "DeleteAllDisposedRuleByBundle appId:%{public}s failed.", appId.c_str());
        return ERR_BUNDLE_MANAGER_APP_CONTROL_INTERNAL_ERROR;
    }
    return ERR_OK;
}
...
{
    LOG_D(BMSTag::APP_CONTROL, "rdb begin to GetDisposedRule");
    NativeRdb::AbsRdbPredicates absRdbPredicates(APP_CONTROL_RDB_TABLE_NAME);
    absRdbPredicates.EqualTo(CALLING_NAME, callingName);
    absRdbPredicates.EqualTo(APP_CONTROL_LIST, DISPOSED_RULE);
    absRdbPredicates.EqualTo(APP_ID, appId);
    ...
    if (count == 0) {
        LOG_D(BMSTag::APP_CONTROL, "GetDisposedRule size 0");
        return ERR_OK;
    }
    ret = absSharedResultSet->GoToFirstRow();
    if (ret != NativeRdb::E_OK) {
...
{
    LOG_D(BMSTag::APP_CONTROL, "rdb begin to GetAbilityRunningControlRule");
    NativeRdb::AbsRdbPredicates absRdbPredicates(APP_CONTROL_RDB_TABLE_NAME);
    absRdbPredicates.EqualTo(APP_CONTROL_LIST, DISPOSED_RULE);
    absRdbPredicates.EqualTo(APP_ID, appId);
    absRdbPredicates.EqualTo(USER_ID, std::to_string(userId));
    ...
    do {
        ...
        if (!parseRet) {
            LOG_W(BMSTag::APP_CONTROL, "parse DisposedRule failed");
        }
